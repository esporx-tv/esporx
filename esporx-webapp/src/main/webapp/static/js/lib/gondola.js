...
var Gondola = Class.create({
	...
	initialize: function() {
		...
		console.log(this.slides, this.slideControls, this.observableTextAreas);
	},
	...
	duplicateFirstSlide: function() {
		...
		this.slides.last().insert({'after': copy});	
		this.slides = $$(this.gondolaSlideSelector);
	},
	...
	resizeSlides: function() {
		...
		$(this.gondolaContainer).setStyle({width: slideContainerWidth + '%'});
		...
		this.slides.last().setStyle({width: lastSlideWidth  + '%'});
	},
	
	startAnimation: function() {
		var carousel = new Carousel(this.gondolaRootId, this.slides, this.slideControls, {
			auto: true,
			circular: true,
			wheel: false,
			frequency: 3
