...
Control.DatePicker.prototype = {
	initialize : function(element, options) {
		...
		this.i18n = new Control.DatePicker.i18n(
				options && options.locale ? options.locale : 'en_US');
		...
		// this.options.currentFormat = this.options.timePicker ?
		// this.options.dateTimeFormat : this.options.dateFormat;
		this.options.date = DateFormat.parseFormat(this.element.value,
				this.options.currentFormat);
		this.options.offsetBottom = options.offsetBottom ? options.offsetBottom
				: 0;
		this.options.offsetLeft = options.offsetLeft ? options.offsetLeft : 0;

		// Lazy load to avoid excessive CPU usage with lots of controls on one
		// page
		...
		if (this.options.icon) {
			this.element.style.background = 'url(' + this.options.icon
					+ ') right center no-repeat #FFF';
			// Prevent text writing over icon
			this.element.style.paddingRight = '20px';
		}
		Event.observe(this.element, 'click', this.togglePicker
				.bindAsEventListener(this));
		if (this.options.iconElement) {
			Event.observe(this.options.iconElement, 'click', this.togglePicker
					.bindAsEventListener(this));
		}

		this.hidePickerListener = this.delayedHide.bindAsEventListener(this);
		Event.observe(this.element, 'keydown', this.keyHandler
				.bindAsEventListener(this));
		Event.observe(document, 'keydown', this.docKeyHandler
				.bindAsEventListener(this));
	...
	datePicked : function(date) {
		this.element.value = DateFormat
				.format(date, this.options.currentFormat);
		this.element.focus();
		this.hide();
	...
	dateHover : function(date) {
		...
		if (this.pickerActive) {
			this.element.value = DateFormat.format(date,
					this.options.currentFormat);
	...
	hide : function() {
		if (this.pickerActive && !this.element.disabled) {
			...
			Event.stopObserving(document, 'click', this.hidePickerListener,
					true);
			this.pickerActive = false;
			Control.DatePicker.activePicker = null;
		}
	...
	show : function() {
		if (!this.pickerActive) {
			...
			/*
			 * var pos = this.element.cumulativeOffset(); if (!/MSIE
			 * 8/.test(navigator.userAgent)) { // IE seems to account for
			 * scrollTop in offsetTop already var scroll =
			 * this.scrollOffset(this.element); pos[0] -= scroll[0] +
			 * document.body.scrollTop; pos[1] -= scroll[1] +
			 * document.body.scrollLeft; }
			 */
			var pos = Position.positionedOffset(this.element);
			var dim = Element.getDimensions(this.element);
			var pickerTop = /MSIE/.test(navigator.userAgent) ? (pos[1] + dim.height)
					+ 'px'
					: (pos[1] + dim.height - 1) + 'px';
			this.datepicker.element.style.position = 'absolute';
			if (this.options.offsetBottom) {
				this.datepicker.element.style.bottom = this.options.offsetBottom
						+ 'px';
			} else {
				this.datepicker.element.style.top = pickerTop;
			}
			this.datepicker.element.style.left = (pos[0] + this.options.offsetLeft)
					+ 'px';
			this.datepicker.element.style.zIndex = '99';
			this.datepicker.selectDate(DateFormat.parseFormat(
...
Object.extend(Control.DatePicker.i18n, {
	...
	createLocale : function(base, lang) {
		return Object.extend(Object
				.clone(Control.DatePicker.i18n.baseLocales[base]), {
			'language' : lang
		});
	}
});
Control.DatePicker.i18n.prototype = {
	initialize : function(code) {
		...
		var language = locale ? Control.DatePicker.Language[locale.language]
				: null;
		if (language)
			Object.extend(this.opts, language);
	},
	opts : null,
	...
	tr : function(str) {
		return this.opts && this.opts.strings ? this.opts.strings[str] || str
				: str;
	}
};
...
with (Control.DatePicker) {
	// Full locale definitions not needed if countries use the language default
	// format
	// Datepicker will fallback to the language default; i.e. 'es_AR' will use
	// 'es'
	Locale['es'] = i18n.createLocale('eu', 'es');
	Locale['en'] = i18n.createLocale('us', 'en');
	Locale['en_GB'] = i18n.createLocale('eu', 'en');
	Locale['en_US'] = i18n.createLocale('us', 'en');
	Locale['fr_FR'] = i18n.createLocale('eu', 'fr');
	Locale['it_IT'] = i18n.createLocale('eu', 'it');
	Locale['en_AU'] = Locale['en_GB'];
	Locale['de'] = i18n.createLocale('eu', 'de');
	Locale['nl'] = i18n.createLocale('eu', 'nl');
...
Control.DatePicker.Language = {
	...
	'it' : {
		months : [ 'Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio',
				...
				'Novembre', 'Dicembre' ],
		days : [ 'Di', 'Lu', 'Ma', 'Me', 'Gi', 'Ve', 'Sa' ],
		strings : {
			'Now' : 'Ora',
			'Today' : 'Oggi',
			'Time' : 'Ore',
			'Exact minutes' : 'Minuti esatti',
			'Select Date and Time' : 'Scegli data e ore',
			'Select Time' : 'Scegli ore',
...
Object
		.extend(
				Control.DatePickerPanel.prototype,
				{
					initialize : function(options) {
						this.i18n = new Control.DatePicker.i18n(options
						...
						this.currentDate = this.options.date ? this.options.date
								: new Date();
						this.dayOfWeek = 0;
						this.minInterval = 5;
					...
					createPicker : function() {
						...
						this.calendarCont = this.drawCalendar(elt,
								this.currentDate);

						Event.observe(elt, 'click', this.clickHandler
								.bindAsEventListener(this));
						Event.observe(elt, 'dblclick', this.dblClickHandler
								.bindAsEventListener(this));
					...
					setDate : function(date) {
						if (date) {
							...
							while (this.element.firstChild)
								this.element
										.removeChild(this.element.firstChild);
							this.calendarCont = this.drawCalendar(this.element,
									date);
						}
					},
						...
						if (this.options.timePicker) {
							...
							for ( var j = 0; j < hourrows; ++j) {
								...
								for ( var i = 0; i < 6; ++i) {
									...
									cell.className = 'hour';
									cell.width = '14%';
									cell.innerHTML = (j * 6) + i
											+ (this.options.use24hrs ? 0 : 1);
									cell.onclick = this
											.hourClickedListener((j * 6)
													+ i
													+ (this.options.use24hrs ? 0
															: 1));
							...
							cell.colSpan = 5;
							cell.innerHTML = '<i>' + this.tr('Exact minutes')
									+ ':</i>';
							...
							// Remove event key capture to allow use of arrow
							// keys
							otherInput.onclick = otherInput.select;
							otherInput.onfocus = this.releaseKeys
									.bindAsEventListener(this);
							otherInput.onblur = this.captureKeys
									.bindAsEventListener(this);
					...
					createCalendar : function(date) {
						...
						var today = new Date();
						var previousYear = new Date(date.getFullYear() - 1,
								date.getMonth(), 1)
						var previousMonth = new Date(date.getFullYear(), date
								.getMonth() - 1, 1)
						var nextMonth = new Date(date.getFullYear(), date
								.getMonth() + 1, 1)
						var nextYear = new Date(date.getFullYear() + 1, date
								.getMonth(), 1)
						...
						cell = row.insertCell(0);
						cell.colSpan = 7;
						cell.innerHTML = this.monthName(date.getMonth()) + ' '
								+ date.getFullYear();
						...
						cell.title = this.monthName(previousYear.getMonth())
						...
						cell = row.insertCell(1);
						cell.className = 'navbutton';
						cell.title = this.monthName(previousMonth.getMonth())
						...
						cell.className = 'navbutton';
						cell.title = this.monthName(nextMonth.getMonth()) + ' '
						...
						cell.className = 'navbutton';
						cell.title = this.monthName(nextYear.getMonth()) + ' '
						...
						for ( var i = 0; i < 7; ++i) {
							...
							cell.width = '14%';
							cell.innerHTML = this
									.dayName((this.options.firstWeekDay + i) % 7);
						}

						row = null;
						var workDate = new Date(date.getFullYear(), date
								.getMonth(), 1);
						...
						if (day != this.options.firstWeekDay) {
							...
							workDate
									.setDate(workDate.getDate()
											- ((day - this.options.firstWeekDay + 7) % 7));
							day = workDate.getDay();
							while (workDate.getMonth() != date.getMonth()) {
								cell = row.insertCell(row.cells.length);
								this.assignDayClasses(cell, 'dayothermonth',
										workDate);
								cell.innerHTML = workDate.getDate();
								cell.onclick = this
										.dateClickedListener(workDate);
					...
					movePreviousMonthListener : function() {
						return function(e) {
							var prevMonth = new Date(this.currentDate
									.getFullYear(),
									this.currentDate.getMonth() - 1,
									this.currentDate.getDate(),
									this.currentDate.getHours(),
									this.currentDate.getMinutes());
							if (prevMonth.getMonth() != (this.currentDate
									.getMonth() + 11) % 12)
								prevMonth.setDate(0);
					...
					moveNextMonthListener : function() {
						return function(e) {
							var nextMonth = new Date(this.currentDate
									...
									this.currentDate.getHours(),
									this.currentDate.getMinutes());
							if (nextMonth.getMonth() != (this.currentDate
									.getMonth() + 1) % 12)
								nextMonth.setDate(0);
					...
					moveNextYearListener : function() {
						return function(e) {
							var nextYear = new Date(this.currentDate
									.getFullYear() + 1, this.currentDate
									.getMonth(), this.currentDate.getDate(),
									this.currentDate.getHours(),
									this.currentDate.getMinutes());
							if (nextYear.getMonth() != this.currentDate
									.getMonth())
								nextYear.setDate(0);
					...
					movePreviousYearListener : function() {
						return function(e) {
							var prevYear = new Date(this.currentDate
									.getFullYear() - 1, this.currentDate
									.getMonth(), this.currentDate.getDate(),
									this.currentDate.getHours(),
									this.currentDate.getMinutes());
							if (prevYear.getMonth() != this.currentDate
									.getMonth())
								prevYear.setDate(0);
					...
					dateClickedListener : function(date, timeOverride) {
						...
						return function(e) {
							if (!timeOverride) {
								...
								dateCopy.setMinutes(this.currentDate
										.getMinutes());
							}
					...
					amClickedListener : function() {
						return function(e) {
							if (this.selectedAmPm == this.pmCell) {
								this.currentDate.setHours(this.currentDate
										.getHours() - 12);
								this.dateClicked(this.currentDate);
							}
						}.bindAsEventListener(this);
					...
					keyHandler : function(e) {
						...
						case Event.KEY_DOWN:
							...
							break;
						case 33: // PgUp
							var lastMonth = new Date(this.currentDate
									.getFullYear(),
									this.currentDate.getMonth() - 1,
									this.currentDate.getDate());
						...
						if (days != 0) {
							var moveDate = new Date(this.currentDate
									.getFullYear(),
					...
					selectDate : function(date) {
						if (date) {
							if (this.options.datePicker) {
								if (date.getMonth() != this.currentDate
										.getMonth()
										|| date.getFullYear() != this.currentDate
												.getFullYear())
									this.setDate(date);
								else
									this.currentDate = date;
								...
								if (date.getDate() < this.currentDays.length) {
									...
									this.selectedDay = this.currentDays[date
											.getDate()];
									Element.addClassName(this.selectedDay,
											'current');
								}
							}
							...
							if (this.options.timePicker) {
								...
								if (this.selectedHour)
									Element.removeClassName(this.selectedHour,
											'current');
								if (this.options.use24hrs)
									this.selectedHour = this.hourCells[hours];
								else
									this.selectedHour = this.hourCells[hours % 12 ? (hours % 12) - 1
											: 11];
								Element.addClassName(this.selectedHour,
										'current');

								if (this.selectedAmPm)
									Element.removeClassName(this.selectedAmPm,
											'current');
								this.selectedAmPm = (hours < 12 ? this.amCell
										: this.pmCell);
								Element.addClassName(this.selectedAmPm,
										'current');

								var minutes = date.getMinutes();
								if (this.selectedMinute)
									Element.removeClassName(
											this.selectedMinute, 'current');
								Element.removeClassName(this.otherMinutes,
										'current');
								if (minutes % this.minInterval == 0) {
									this.otherMinutes.value = '';
									this.selectedMinute = this.minuteCells[minutes
											/ this.minInterval];
									Element.addClassName(this.selectedMinute,
											'current');
								} else {
									this.otherMinutes.value = minutes;
									Element.addClassName(this.otherMinutes,
											'current');
								}
							}
